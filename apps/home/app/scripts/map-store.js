...
module.exports = class MapStore {
  ...
  constructor(map_actions) {
    ...
    this.data = immutable.Map({'build_type': 'flats',
                               ...
                               'show_help_popups': this.calculateShowHelpPopupsFromQueryString(),
                               'scenario': this.calculateScenarioFromQueryString()});
  }
  ...
  init(data) {
    ...
    this.listenTo(this.map_actions.clear_all,
                  this.onClearAll);
    this.listenTo(this.map_actions.first_load,
                  this.onFirstLoad);
  ...
  getShowHelpPopups() {
    return this.data.get('show_help_popups');
  }
  
  getScenario() {
    return this.data.get('scenario');
  }
  ...
  calculateShowHelpPopupsFromQueryString() {
    var helpPopups = this.getParameterByName('hp');
    if(helpPopups) {
      return helpPopups == '1';
    } else {
      return true;
  ...
  calculateScenarioFromQueryString() {
    var scenario = this.getParameterByName('s');
    if(scenario) {
      return scenarios.getScenarioById(scenario);
    } else {
      return scenarios.getDefault();
    }
  }
  ...
  createHomesBuiltQueryString(buildFeatures) {
    ...
    var params = {'fb': this.getBaseline().get('flats'),
                  ...
                  's': this.getScenario().id,
                  'hp': this.getShowHelpPopups() ? '1': '0',
                  'datav1': base64Data
                  };
